...
const { $categories, $auth } = useNuxtApp()
const { data , pagination, months, queryParams,  status, isLoading} = storeToRefs($categories)
...
async function  getCategories() {
    await $categories.get(query.value, perPage.value, page.value)
}
...
async function addedToLibrary () {
    ...
    isShowModalCategoryCreate.value = false
    isShowModalCategoryEdit.value = false
}
...
async function destroy(item) {
    if (confirm(`Czy na pewno chcesz usunąć ten plik?`)) {     
        await $categories.destroy(item.id)
        await getCategories()
    }
}

async function executeAction(actionId) {
    const categoriesIds = data.value.filter(item => item.selected)
        .map(item => item.id);
    ...
    switch (actionId) {
        ...
        if (confirm(`Czy jesteś pewny, że chcesz usunąć?`)) {
            categoriesIds.forEach(async (categoriesId) => {                    
                const deleted = await $categories.destroy(categoriesId)
...
<template>
    <x-section-dashboard v-if="$auth.accessGranted('ROLE_ADMIN')"  :container="false">
        <template #header-panel>
            ...
            <x-btn
            @click="isShowModalCategoryCreate = true"
                class="h-9 group"
        ...
        <template #main>
            <div class=" h-full p-6 lg:p-10 box-border dark:bg-gray-800/20 transition-all duration-500 rounded-xl" >
                ...
                <div v-if="data" class="w-full h-full flex">
                    <div class="transition-all duration-500 w-full" >
                        <div class="w-full h-full transition-all duration-500" >
                            <x-table
                                :head="['name', 'slug', 'color', 'description', 'status', 'created', 'updated', '' ]"
                            ...
                            >
                                ...
                                <x-table-body v-for="(item, index) in data" :key="item.index = index" :class="item.status == 0 ? 'bg-black/30' : ''">
                                    ...
                                    <x-table-body-cell justify="center">
                                        <div class="w-6 h-6 rounded-full" :style="{
                                ...
                                <template #pagination>
                                    <x-pagination :count="data?.length" :pagination="pagination"  @page="switchPage" @per_page="switchPerPage" :loading="isLoading" />
                                </template>
                            </x-table>
                        </div>
                    </div>
                ...
                <x-modal-category-create
                    ...
                    :title="`Creating a new description in categories section.`"
                />
                ...
                <x-modal-category-edit
                    ...
                    @addedToLibrary="addedToLibrary"
                    :title="`Edit: ${categoryPreview?.name}`"
                />
            </div>
        </template>
