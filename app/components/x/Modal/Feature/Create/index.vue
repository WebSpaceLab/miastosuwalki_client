    ...
    >
        <form class="w-full h-full flex flex-col justify-center items-start space-y-6 py-6">
            <div class="w-full h-60 flex justify-center items-start space-x-6">
                <div class="w-full h-full">
                    ...
                    <p v-if="$feature.errors && $feature.errors.mediaId " class="text-sm text-error-600 m-0 ">{{ $feature.errors.mediaId }}</p>
                </div>
                ...
                <div class="w-full h-full flex flex-col justify-between items-start space-y-6">
                    ...
                    <div class="flex flex-row justify-center items-center w-full h-9 mb-6">
                        <div class="w-1/3 h-full">
                            ...
                            <p v-if="$feature.errors && $feature.errors.isActive " class="text-sm text-error-600 m-0 ">{{ $feature.errors.isActive }}</p>
                        </div>
                    </div>
                </div>
            </div>
       ...
       <template #footer>
            <div class="flex space-x-3">
                ...
                <x-btn @click.prevent="create" @keydown.enter="create" type="submit" color="primary-outline" rounded :loading="$feature.isLoading" :disabled="!form.name || !form.mediaId">Create</x-btn>
            </div>
       </template>
    </x-modal-dialog>
</template>