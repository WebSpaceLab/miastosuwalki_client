...
const update = async () => {
    try {
        await $team.update(teamPreview.value.id, form)
...
watch(() => props.show, () => {
    teamPreview.value = props.preview
    ...
    >
       ...
       <template #footer>
            <div class="flex space-x-3">
                <x-btn @click="update" @keydown.enter="update" type="submit" color="primary-outline" rounded :loading="$team.isLoading" :disabled="!form.name || !form.mediaId">Update</x-btn>
            </div>
       </template>
    </x-modal-dialog>
</template>