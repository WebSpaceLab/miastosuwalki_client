...
const form = reactive({
    info: props.pricePackageInformation ? props.pricePackageInformation.info : '',
    isHighlighted: props.pricePackageInformation ? props.pricePackageInformation.isHighlighted : false,
})
...
async function updatedPricePackageInformation() {
   const data = await $price.updatedPricePackageInformation(form, props.pricePackageInformation.id)
...
watch(() => props.pricePackageInformation, (pricePackageInformation) => {
    if(pricePackageInformation) {
        form.info = pricePackageInformation.info
        form.isHighlighted = pricePackageInformation.isHighlighted
    }
})
</script>
...
<template>
    <div class="w-full flex justify-center items-center p-6 box-border" >
        <form  class="relative w-full h-full flex flex-col space-y-6" @submit.prevent="updatedPricePackageInformation()">
            <x-input
                class="mb-4"
                label="Tytuł"
                v-model="form.info"
