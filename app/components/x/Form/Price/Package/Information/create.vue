...
async function createdPricePackageInformation() {
   const data = await $price.createdPricePackageInformation(form, props.pricePackageId)
...
<template>
    <div class="w-full flex justify-center items-center p-6 box-border" >
        <form  class="relative w-full h-full flex flex-col space-y-6" @submit.prevent="createdPricePackageInformation()">
            <x-input
                class="mb-4"
                label="Tytuł"
                v-model="form.info"
