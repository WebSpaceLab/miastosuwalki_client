...
const props = withDefaults(defineProps<{ title?: string, description?: string, headline?: string, siteLogo?: string }>(), {
  title: 'title',
...
<template>
  <div class="w-full h-full flex flex-col justify-center bg-[#020420]">
    ...
    <div class="w-[700px] pl-[100px]">
      ...
      <h1 class="w-[700px] m-0 text-[65px] font-semibold mb-4 text-white flex items-center">
        <span>{{ title }}</span>
      </h1>
      <p class="w-full text-[32px] text-[#E4E4E7] leading-tight">
        {{ description.slice(0, 200) }}
      </p>
    </div>

    <div  class="absolute top-[50px] right-[50px]">
      <img :src="siteLogo" alt="Logo"  class="w-90 h-90 rounded-full" />
    </div>

   <!--
    <div  class="absolute bottom-[50px] right-[80px]">
        <img :src="icon" alt="Icon"  class="w-90  rounded-full" />
      </div>
  --> 
  </div>
</template>