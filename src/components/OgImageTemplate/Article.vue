...
const props = withDefaults(defineProps<{ title?: string, description?: string, headline?: string, image?: string, icon?: string }>(), {
  title: 'title',
...
<template>
  <div class="w-full h-full flex flex-col justify-start bg-[#020420] py-10">
    <div  class="absolute  top-[10px] left-[10px] flex justify-center">
      <img :src="icon" alt="Icon"  class="w-30 h-30 rounded-full" />
    </div>

    <div  class="absolute bottom-[30px] right-[50px]">
      <img :src="image" alt="Logo"  class="w-100  object-cover rounded-xl" />
    </div>
    <svg
    ...
    <div class="w-[700px] pl-[100px]">
      <p v-if="headline" class="pl-10 uppercase text-2xl text-[#00DC82] mb-4 font-semibold">
        {{ headline }}
      </p>

      <h1 class="w-[700px] m-5 text-5xl font-semibold mb-4 text-white flex items-center text-wrap">
        <span >{{ title }}</span>
      </h1>
      <p class="w-full text-3xl text-[#E4E4E7] leading-tight">
