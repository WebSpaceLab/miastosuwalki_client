...
const addedToLibrary = async (event) => {
    form.mediaId = event.id
    console.log(form.mediaId)
    // emits('addedToLibrary', true)
}
    ...
    >
       ...
       <template #footer>
            <div class="flex space-x-3">
                <x-btn @click="update" @keydown.enter="update" type="submit" color="primary-outline" rounded :loading="$about.isLoading" :disabled="!form.name || !form.mediaId">Update</x-btn>
            </div>
       </template>
    </x-modal-dialog>
</template>