    ...
    >
        <form class="w-full h-full flex flex-col justify-center items-start space-y-6 py-6">
            <div class="w-full h-60 flex justify-center items-start space-x-6">
                <div class="w-full h-full">
                    ...
                    <p v-if="$team.errors && $team.errors.mediaId " class="text-sm text-error-600 m-0 ">{{ $team.errors.mediaId }}</p>
                </div>
                ...
                <div class="w-full h-full flex flex-col justify-between items-start space-y-6">
                    ...
                    <div class="w-full ">
                        <x-input
                            ...
                            color="blue"
                            label="Name"
                            :error="$team.errors && $team.errors.name ? $team.errors.name : ''"
                        />
                    ...
                    <div class="flex flex-row justify-center items-center w-full h-9 mb-6">
                        <div class="w-1/3 h-full">
                            ...
                            <p v-if="$team.errors && $team.errors.isActive " class="text-sm text-error-600 m-0 ">{{ $team.errors.isActive }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="w-full">
                ...
                <div class="flex w-full justify-start items-center space-x-4">
                    <x-textarea
                        ...
                        label="Description"
                        :error="$team.errors && $team.errors.description ? $team.errors.description : ''"
                        :rows="5"
                    />
                </div>
            </div>
       ...
       <template #footer>
            <div class="flex space-x-3">
                ...
                <x-btn @click.prevent="create" @keydown.enter="create" type="submit" color="primary-outline" rounded :loading="$team.isLoading" :disabled="!form.name || !form.mediaId">Create</x-btn>
            </div>
       </template>
    </x-modal-dialog>
</template>