...
const { allPublished , category, pagination, months, queryParams } = storeToRefs($article)
...
async function  getPublishedArticlesFromCategory() {
    await $article.getPublishedArticlesFromCategory(slug, query.value, perPage.value, page.value)
}


async function switchPage(event) {
    page.value = event
    await getPublishedArticlesFromCategory()
}
...
<template>
    <section class="w-screen min-h-screen container flex flex-col justify-start items-start pt-5 px-6">
        ...
        <div  class="w-full h-full pt-6 box-border ">
            <header class="w-full flex flex-col box-border ">
                <div class="mb-1 w-full  flex justify-center items-center rounded-t-lg" :style="{
                    backgroundColor: category.color ? category.color : '#000000',
                }">
                    <h2 class="text-2xl m-0 text-background-light dark:text-background-dark uppercase font-extrabold">{{ slug }}</h2>
                </div>
    
                <span class="border border-solid border-b-1" :style="{
                    borderColor: category.color ? category.color : '#000000',
                }"/>
            </header>
    
    
            <div class=" h-full py-6 lg:p-10 box-border dark:bg-gray-800/20 transition-all duration-500 rounded-xl" >
                <div v-if="allPublished" class="w-full h-full flex">
                    <div class="transition-all duration-500 w-full" >
                        <div v-if="allPublished.length != 0" class="w-full h-full transition-all duration-500" >
                            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 xl:gap-4 xl:px-6">
